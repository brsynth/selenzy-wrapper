...
def fingerprint():
    ...
    fpd = {
        "Pattern": ("ptfp", None, True, PatternFingerprint, 2),
        "RDK": ("rdkfp", None, True, RDKFingerprint, 1),
...
def getClosest(
    smi, fpfile, th=0.8, fp=None, fpn=None, fpp=None, fpfun=None, marvin=False
):
    dist = {}
    if fp is None:
    ...
    if targetMol is None and marvin:
        try:
            ...
            job2 = subprocess.Popen(
                cmd2, stdin=job.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            job.stdout.close()
...
def arguments(args=None):
    parser = argparse.ArgumentParser(
        description="quickRSim Pablo Carbonell, SYNBIOCHEM, 2016"
    )
    parser.add_argument("datadir", help="Data folder")
    parser.add_argument("fp", help="Fingerprint for reactants")
    parser.add_argument("-rxn", help="Input reaction rxn file")
    parser.add_argument("-rid", help="Input reaction id")
    parser.add_argument("-smarts", help="Input reaction SMARTS")
    parser.add_argument("-smartsfile", help="Input reaction SMARTS file")
    parser.add_argument(
        "-chem", help="Metanetx chemical structures (if input is reaction id)"
    )
    parser.add_argument(
        "-th", type=float, default=0.8, help="Similarity threshold [default=0.8]"
    )
    parser.add_argument(
        "-out", help="Output results in .txt file, please specify file name"
    )
    parser.add_argument(
        "-high",
    ...
    parser.add_argument("-marvin", help="Call marvin if needed (skip if fails)")
