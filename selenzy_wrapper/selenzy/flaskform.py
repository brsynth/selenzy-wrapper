...
ALLOWED_EXTENSIONS = set(
    ["txt", "rxn", "smi", "smarts", "smirks", "csv", "fasta", "fas", "fa"]
)


def arguments():
    parser = argparse.ArgumentParser(description="Options for the webserver")
    parser.add_argument("-uploaddir", default="uploads", help="Upload folder")
    parser.add_argument(
        "-datadir", default="data", help="Data directory for required databases files"
    )
    parser.add_argument("-logdir", default="log", help="Logging folder")
    parser.add_argument(
        "-d", action="store_true", help="Run in debug mode (no preload)"
    )
...
def run_session(rxntype, rxninfo, targets, direction, host, fp, noMSA):
    ...
    success, app.config["TABLES"] = Selenzy.analyse(
        ["-" + rxntype, rxninfo],
        targets,
...
class RestQuery(Resource):
    ...
    def post(self):
        ...
        if "rxnid" in args and "db" in args and "smarts" not in args:
            ...
            rxnid = db + ":" + args["rxnid"]
            if (
                rxnid in app.config["TABLES"].rxnref
                and app.config["TABLES"].rxnref[rxnid] in app.config["TABLES"].smir
            ):
            ...
            try:
                if isinstance(rxninfo, (list, tuple)):
                    ...
                    for instance in rxninfo:
                        dat, csvfile, sessionid = run_session(
                            rxntype, instance, targets, direction, host, fp, noMSA
                        )
                        data.append(dat)
                    data = pd.DataFrame(data)
                else:
                    data, csvfile, sessionid = run_session(
                        rxntype, rxninfo, targets, direction, host, fp, noMSA
                    )
                return jsonify(
                    {
                        "app": "Selenzy",
...
class RestSource(Resource):
    ...
    def get(self):
        ...
        return jsonify(
            {"app": "Selenzy", "version": "1.0", "author": "Synbiochem", "data": orgs}
        )
...
def upload_form():
    ...
    return render_template(
        ...
        fingerprints=Selenzy.availableFingerprints(),
    )
...
def post_msa():
    ...
    if request.method == "POST":
        ...
        msafile = os.path.join(
            app.config["UPLOAD_FOLDER"], sessionid, "sequences_aln.fasta"
        )
    ...
    if request.method == "POST":
        ...
        if "file" in request.files and len(request.files["file"].filename) > 0:
            ...
            if len(rxninfo) == 0:
                ...
                if marvin:
                    svgstream = Selenzy.display_reaction(
                        rxninfo,
                        outfolder=session["uniquefolder"],
                    ...
                    outfile, size = Selenzy.display_reaction(
                        rxninfo,
                        outfolder=session["uniquefolder"],
                        outname=str(uuid.uuid4()),
                        marvin=False,
            ...
            if marvin:
                ...
                if len(outfile) == 0:
                    success = False
                data = os.path.join(
                    "/results", session["uniqueid"], "files", os.path.basename(outfile)
                )
            session["rxninfo"] = rxninfo
            session["rxntype"] = "smarts"
            session["status"] = True
    ...
    if request.method == "POST":
        ...
        if "fasta" in request.files and len(request.files["fasta"].filename) > 0:
            ...
            if os.path.exists(dndFile):
                ...
                noMSA = True
            csvfile = Selenzy.extend_sequences(
                "sequences.fasta", fastafile, uniquefolder, noMSA
            )
            data = Selenzy.updateScore(
                file_path(uniquefolder, csvfile), session["SCORE"]
            )
...
def delete_rows():
    ...
    if request.method == "POST":
        ...
        Selenzy.write_fasta(
            fastaShortNameFile, newtargets, app.config["TABLES"], short=True
        )
        ...
        if os.path.exists(dndFile):
            ...
            for i in range(0, len(newrows)):
                try:
                    newrows[i][head.index("Consv. Score")] = cons[
                        newrows[i][head.index("Seq. ID")]
                    ]
                except:
                    pass
...
def show_table():
    if app.debug == True:
        csvfile = os.path.join(
            app.config["UPLOAD_FOLDER"], "debug", "selenzy_results.csv"
        )
        data = Selenzy.updateScore(csvfile, session["SCORE"])
        sessionid = "debug"
        data.rename_axis("Select", axis="columns")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template(
                ...
                csvfile=csvfile,
                sessionid=sessionid,
                flags={"fasta": True, "msa": not noMSA},
                score=session["SCORE"],
            )
        except:
            return redirect(url_for("upload_form"))
...
def results_file(sessionid, filename):
    return send_from_directory(
        os.path.join(app.config["UPLOAD_FOLDER"], sessionid), filename
    )
...
if __name__ == "__main__":  # only run server if file is called directly
    ...
    handler = RotatingFileHandler(
        os.path.join(app.config["LOG_FOLDER"], "selenzy.log"),
        maxBytes=10000,
        backupCount=1,
    )
