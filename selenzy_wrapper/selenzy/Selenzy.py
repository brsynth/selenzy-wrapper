...
class preLoad(object):
    ...
    def fasta(self, datadir, ffile="seqs.fasta"):
        (sequence, names, descriptions, fulldescriptions, osource, pexistence) = (
            readFasta(datadir, ffile)
        )
...
def seqScore(newscore=None):
    ...
    vdict = {
        ...
        string.ascii_uppercase[8]: (
            'Sequence <a href="https://en.wikipedia.org/wiki/Conserved_sequence" target="_blank">conservation</a>:',
            1.0,
            True,
        ),
        string.ascii_uppercase[4]: (
            'Sequence <a href="https://www.ncbi.nlm.nih.gov/taxonomy" target="_blank">taxonomic</a> distance:',
            -1.0,
            True,
        ),
        string.ascii_uppercase[7]: (
            'Uniprot <a href="http://www.uniprot.org/help/protein_existence" target="_blank">protein evidence</a>:',
            -0.1,
            True,
        ),
...
def getMnxSim(rxnInput, datadir, outdir, drxn=0, fp="RDK", pc=None):
    ...
    args = (
        [datadir, fp]
        + rxnInput
        + ["-out", os.path.join(outdir, "results_quickRsim.txt")]
    )
    ...
    for record in SeqIO.parse(infile, "fasta"):
        ...
        for aa in record.seq:
            if aa not in protein_letters_1to3:
                ...
                newseq += aa
        newrecords.append(
            SeqRecord(Seq(newseq), id=record.id, description=record.description)
        )
...
def doMSA(finallistfile, outdir):
    ...
    args = (
        "t_coffee -in {0} -mode quickaln -output=score_ascii,fasta_aln,score_html -outfile ".format(
            finallistfile
        )
        + outfile
        + " -newtree "
...
def extend_sequences(initialfastafile, fastafile, workfolder, noMSA):
    ...
    try:
        ...
        (sequence, names, descriptions, fulldescriptions, osource, pexistence) = (
            readFasta(workfolder, fastafile, limit=1000)
        )

        if len(sequence) == 0:
            return csvfile

        (hydrop, weight, isoelec, polar, helices, sheets, turns, coils) = (
            sequence_properties(os.path.join(workfolder, shortfile))
        )
...
def sequence_properties(fastaShortNameFile):
    ...
    (hydrop, weight, isoelec, polar) = pepstats(
        fastaShortNameFile, os.path.dirname(fastaShortNameFile)
    )
    (helices, sheets, turns, coils) = garnier(
        fastaShortNameFile, os.path.dirname(fastaShortNameFile)
    )
...
def analyse(
    ...
    try:
        (MnxSim, MnxDirPref, MnxDirUsed, Smiles, EcNumber) = getMnxSim(
            rxnInput, datadir, outdir, pdir, fp, pc
        )
    except:
        return False, pc
    ...
    list_mnx = sorted(
        MnxSim, key=lambda y: (-float(MnxSim[y]), y), reverse=False
    )  # allow user to manipulate window of initial rxn id list
    ...
    for x in list_mnx:
        ...
        if up is not None:
            ...
            for y in up:
                if len(targets) >= int(
                    targ
                ):  # allow user to manipulate desired number of entries for resulting table
                    break
                else:
                    UprotToMnx[y] = x
                    try:
    ...
    (hydrop, weight, isoelec, polar, helices, sheets, turns, coils) = (
        sequence_properties(fastaShortNameFile)
    )
    if not NoMSA:
...
def arguments():
    ...
    parser.add_argument("rxn", help="Input reaction [default = rxn file]")
    parser.add_argument(
        "-tar",
        type=float,
        default=20,
    ...
    parser.add_argument("-outfile", help="specify non-default name for CSV file output")
    parser.add_argument(
        "-NoMSA", action="store_true", help="Do not compute MSA/conservation scores"
    )
    parser.add_argument(
        "-smarts", action="store_true", help="Input is a reaction SMARTS string"
    )
    parser.add_argument(
        "-smartsfile", action="store_true", help="Input is a reaction SMARTS file"
    )
    parser.add_argument(
        "-host",
        type=str,
