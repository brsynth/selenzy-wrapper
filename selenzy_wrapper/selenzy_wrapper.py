...
sys_path.insert(0, os_path.join(__PACKAGE_FOLDER, __SELENZY_FOLDER))

__DATA_URL = (
    "https://gitlab.com/breakthewall/rrcache-data/-/raw/master/selenzy/data.tar.gz"
)
...
def selenzy_pathway(
    ...
    if not os_path.exists(datadir_data):
        ...
        download_and_extract_tar_gz(__DATA_URL, datadir)
    ...
    for rxn_id, rxn in pathway.get_reactions().items():
        try:
            ...
            logger.warning(
                f"Problem with retreiving the selenzyme information for pathway {pathway.get_id()}"
            )
        ...
        sorted_ids = dict(
            sorted(ids.items(), key=lambda item: item[1]["score"], reverse=True)[
                :nb_ids
            ]  # keep only IDs with top scores
        )
...
def Selenzy_infos(
    ...
    with open(os_path.join(outdir, "new.csv")) as csv_file:
        df = pd_read_csv(csv_file, delimiter=",")
        ...
        if len(taxonIDs) == 1:
            for index, row in df.iterrows():
                infos[row["Seq. ID"]] = set_infos(
                    score=row["Score"], target_id=row["target_ID"], logger=logger
                )
...
def set_infos(
    score: float, target_id: str, logger: Logger = getLogger(__name__)
) -> Dict:
    return {"score": score, "target_ID": target_id}
